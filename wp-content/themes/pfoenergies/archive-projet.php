    ...
    <div class="max-w-7xl mx-auto py-8">
        ...
        <form method="GET" class="flex items-center justify-center font-light italic gap-4">
            <label class="relative cursor-pointer text-gray-500
                ...
                for="all">
                <input type="radio" name="status" id="all" value="all" class="sr-only" <?= ($_GET['status'] ?? 'all') === 'all' ? 'checked' : '' ?> onchange="this.form.submit()">Tous les projets
            </label>
            <label class="relative cursor-pointer text-gray-500
                after:content-['/_'] after:ml-2 
                has-checked:text-primary has-checked:before:content-[''] has-checked:before:absolute has-checked:before:-bottom-1 has-checked:before:bg-primary has-checked:before:w-5/6 has-checked:before:h-0.5" 
                for="en-cours">
                <input type="radio" name="status" id="en-cours" value="en-cours" class="sr-only" <?= ($_GET['status'] ?? '') === 'en-cours' ? 'checked' : '' ?> onchange="this.form.submit()">En cours 
            </label>
            <label class="relative cursor-pointer text-gray-500
                has-checked:text-primary has-checked:before:content-[''] has-checked:before:absolute has-checked:before:-bottom-1 has-checked:before:bg-primary has-checked:before:w-5/6 has-checked:before:h-0.5" 
                for="acheve">
                <input type="radio" name="status" id="acheve" value="acheve" class="sr-only" <?= ($_GET['status'] ?? '') === 'acheve' ? 'checked' : '' ?> onchange="this.form.submit()">Achevé
            </label>
        </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-8">
            <?php if (have_posts()) : ?>
                ...
                <?php get_template_part('template-parts/projet') ?>
